...
import type { AppAction } from "gloomberb/react";
...
} from "gloomberb/tickers";
import { getSortValue, type ColumnContext } from "gloomberb/components";
...
async function resolveCandidateTicker(
  ...
  const tickerRepository = getTickerRepository();
  if (!tickerRepository || !dataProvider) {
  ...
  const { ticker, created } = await upsertTickerFromSearchResult(tickerRepository, selected);
...
export async function validateScreenerResults(
  ...
  if (!getTickerRepository() || !dataProvider) throw new Error("AI screener could not access the ticker repository.");
